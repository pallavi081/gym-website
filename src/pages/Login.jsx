...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    login('mock-token', { name: 'Member', email });
    navigate('/');
  };
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center px-6 pt-24">
      ...
      >
        <h2 className="text-3xl font-bold mb-8 text-center uppercase tracking-tighter">Login to <span className="text-primary">PowerFit</span></h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest mb-2 text-gray-400">Password</label>
            <input
              type="password"
              value={password}
        ...
        <p className="mt-8 text-center text-gray-500 text-sm">
          Don't have an account? <a href="/signup" className="text-primary hover:underline">Join now</a>
        </p>
      </motion.div>
    </div>
  );
